...
class Signup
{
    ...
    public function evaluate($data)
    {
        ...
        if (strlen($data['username']) > 8) {
            $this->error .= "Username cannot be more than 8 characters!<br>";
        }

        // Validate password length
        if (strlen($data['password']) > 8) {
            $this->error .= "Password cannot be more than 8 characters!<br>";
        }
    ...
    // $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        ...
        if ($_FILES['image_path']['error'] !== UPLOAD_ERR_NO_FILE) {
            ...
            if (in_array($fileType, $validImageExtensions) && $fileSize <= 10000000) {
                ...
                $query = "INSERT INTO users (user_id, username, name, email, password, bio, image_path, account_type, badge, score, level, completed_courses, fb, tw, yt)
                          VALUES ('$userId', '$username', '$name', '$email', '$password', 'Learning Enthusiast | Comprehensive Learning Fan', '$newImageName', '$account_type', 'Normal', '$score', '$level', '$completed_courses', 'fb', 'tw', 'yt')";
                $DB = new Database();
                $DB->save($query);
